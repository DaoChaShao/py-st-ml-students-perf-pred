...
+ **数据导入与处理**：支持 CSV 文件导入，自动分离特征与标签并进行预处理。
+ **特征分析**：基于相关性分析和 PCA 提供特征重要性排序，帮助理解关键因素。
+ **成绩预测**：基于训练好的多层感知机 (MLP) 模型，预测学生成绩（如：期末 G3 成绩）。
+ **误差分析**：提供 MSE、MAE、RMSE 等指标，评估预测结果的准确性。
+ **模型加载与保存**：支持加载已训练模型，保持预测一致性；支持保存训练好的模型。
+ **可视化展示**：可视化特征重要性、预测与真实值对比。
+ **多平台兼容**：可在桌面端运行，支持 GPU 加速和批量计算。
